...
7. SEM POSOLOGIA: Não informar doses, frequência ou esquemas prescritos (ex.: mg, ml, 8/8h).
8. SEM DIAGNÓSTICO FECHADO: Não usar linguagem de certeza diagnóstica definitiva.
...
export const NEGATIVE_MISMATCH_PROMPT = `# DETECÇÃO DE INCOERÊNCIA CLÍNICA (NEGATIVOS)
Objetivo: identificar quando uma hipótese NÃO se assemelha ao quadro informado.

Regras:
1. Use somente sintomas explícitos no caso.
2. Para cada hipótese, marque "incompatível" quando faltarem sintomas-chave mandatórios.
3. Não invente sintomas ausentes para justificar hipótese.
4. Priorize epidemiologia e contexto (idade, sexo e duração).
5. Saída curta e direta, sem links.
