...
describe('backend server routes', () => {
  ...
  it('GET /api/evidence/coverage aponta cobertura e lacunas internas', async () => {
    const app = createApp();
    const response = await request(app)
      .get('/api/evidence/coverage')
      .query({ q: 'dor abdominal vômitos febre', domain: 'gastrointestinal', context: 'emergência' });

    expect(response.status).toBe(200);
    expect(response.body.evidenceCount).toBeGreaterThan(0);
    expect(response.body.coverageScore).toBeGreaterThan(0);
    expect(response.body.matchedDimensions.length).toBeGreaterThan(0);
    expect(response.body.suggestedQueries.length).toBeGreaterThan(0);
    expect(response.body.safety).toContain('lacunas');
  });


  it('POST /api/evidence/case-context conecta caso clínico à base interna', async () => {
    const app = createApp();
    const response = await request(app).post('/api/evidence/case-context').send({
      symptoms: 'Dor torácica com sudorese e dispneia há 1 hora',
      age: 67,
      duration: '1h',
      triageLevel: 'Urgente',
      hypotheses: ['Síndrome coronariana aguda'],
      limit: 6,
    });

    expect(response.status).toBe(200);
    expect(response.body.inferred.domain).toBeTruthy();
    expect(response.body.brief.sections.length).toBeGreaterThan(0);
    expect(response.body.coverage.coverageScore).toBeGreaterThan(0);
    expect(response.body.topEvidence.length).toBeGreaterThan(0);
    expect(response.body.safety).toContain('simulação');
  });
