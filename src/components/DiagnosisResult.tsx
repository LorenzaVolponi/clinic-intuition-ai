...
export const DiagnosisResult = ({ diagnosis, patientData, onReset }: DiagnosisResultProps) => {
  ...
  return (
    <div className="animate-fade-in space-y-5 sm:space-y-6">
      <Card className="overflow-hidden rounded-[2rem] border-white/70 bg-white/90 shadow-2xl shadow-sky-900/10 backdrop-blur-xl">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <CardTitle className="text-xl font-black text-slate-950 sm:text-2xl">Resumo do Paciente</CardTitle>
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              className="h-11 rounded-full border-slate-200 bg-white/80 px-4 font-bold shadow-sm transition-colors hover:bg-primary/10 hover:text-primary lg:self-center"
            >
        ...
        <CardContent className="space-y-5 p-4 sm:p-6 lg:p-8">
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1.25fr)_minmax(18rem,0.75fr)]">
            <div className="rounded-[1.75rem] border border-slate-200/80 bg-slate-50/80 p-4">
              <p className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-slate-500">Sintomas relatados</p>
              <p className="text-sm leading-7 text-slate-700 sm:text-base">{patientData.symptoms}</p>
            </div>

            <div className="rounded-[1.75rem] border border-primary/10 bg-white/85 p-4 shadow-sm">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <Activity className="h-5 w-5 text-primary" />
                  <p className="font-black text-slate-900">Triagem</p>
                </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {summaryStats.map(({ label, value, icon: Icon }) => (
              <div key={label} className="rounded-2xl border border-slate-200/80 bg-white/80 p-4 shadow-sm">
          ...
          {primaryHypothesis && (
            <div className="rounded-[1.75rem] border border-emerald-100 bg-gradient-to-r from-emerald-50 to-cyan-50 p-4 sm:p-5">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">Hipótese líder</p>
                  <p className="mt-1 text-lg font-black text-slate-950">{primaryHypothesis.name}</p>
                </div>
      ...
      {diagnosis.emergencyWarning && (
        <Card className="overflow-hidden rounded-[2rem] border-destructive/40 bg-rose-50/90 shadow-xl shadow-rose-900/10">
          <CardContent className="p-4 sm:p-6">
            <div className="flex items-start gap-3">
              <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-2xl bg-destructive/10 text-destructive">
                <AlertTriangle className="h-6 w-6" />
              </div>
              <div>
                <h3 className="text-lg font-black text-destructive sm:text-xl">Alerta de emergência</h3>
      ...
      <Card className="overflow-hidden rounded-[2rem] border-white/70 bg-white/90 shadow-2xl shadow-sky-900/10 backdrop-blur-xl">
        <CardHeader className="border-b border-slate-200/70 bg-gradient-to-r from-white to-sky-50 p-5 sm:p-7">
          <CardTitle className="flex items-center gap-2 text-xl font-black text-slate-950 sm:text-2xl">
      ...
      <div className="space-y-4">
        {diagnosis.hypotheses.map((hypothesis, index) => {
          ...
          return (
            <Card key={`${hypothesis.name}-${index}`} className="overflow-hidden rounded-[2rem] border-white/70 bg-white/90 shadow-xl shadow-sky-900/10 backdrop-blur-xl">
              <CardHeader className="border-b border-slate-200/70 bg-white/80 p-5 sm:p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-xs font-black uppercase tracking-[0.18em] text-muted-foreground">Hipótese {index + 1}</p>
                      <CardTitle className="mt-1 text-lg font-black text-primary sm:text-xl">{hypothesis.name}</CardTitle>
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant="outline" className="bg-white/70">Score {hypothesis.score}</Badge>
                  </div>
                </div>
      ...
      <Card className="overflow-hidden rounded-[2rem] border-warning/50 bg-gradient-to-r from-warning-soft/80 to-orange-50/80 shadow-xl shadow-amber-900/10">
        <CardContent className="p-4 sm:p-6">
          <div className="flex items-start gap-3">
            <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-2xl bg-warning/20 text-warning">
              <AlertTriangle className="h-6 w-6" />
            </div>
            <div>
              <h3 className="text-lg font-black text-warning sm:text-xl">Aviso educacional importante</h3>
...
const InsightCard = ({ title, icon: Icon, tone, children }: InsightCardProps) => (
  <div className={`rounded-[1.5rem] border p-4 text-sm leading-7 ${toneClasses[tone]}`}>
...
const ListItems = ({ items, emptyText, badge = false }: ListItemsProps) => {
  ...
  if (badge) {
    return (
      <div className="flex flex-wrap gap-2">
        {items.map((item) => (
          <Badge key={item} variant="outline" className="border-current/20 bg-white/70 text-current">
            {item}
          </Badge>
        ))}
      </div>
