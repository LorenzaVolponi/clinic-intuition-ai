...
import { MEDICAL_CONDITIONS, type MedicalCondition } from './medicalKnowledge';
...
export function searchStructuredClinicalKnowledge(query: StructuredClinicalQuery): StructuredClinicalSearchResult {
  ...
  return {
    conditions: MEDICAL_CONDITIONS.filter((condition) => condition.id && conditionIds.has(condition.id)),
    assertions,
