...
export function buildLocalAssessment(patientData: PatientData): ClinicalAssessment {
  ...
  const hypotheses = topMatches.map(({ condition, score }) => ({
    ...
    explanation: `${condition.clinicalPearls[0] ?? 'Raciocínio clínico guiado pelos sintomas predominantes.'} Compatibilidade baseada em ${condition.commonSymptoms.slice(0, 3).join(', ')} e contexto clínico informado. Referência-base: ${condition.category === 'cardiovascular' ? 'SBC Diretrizes' : condition.category === 'neurologico' ? 'AHA/ASA Stroke Guidelines' : 'Diretrizes clínicas reconhecidas'}.`,
  ...
  return {
    ...
    suggestedExams,
    immediateActions,
    clinicalSummary: `Paciente ${patientData.age} anos, ${patientData.gender}, com ${patientData.duration} de sintomas. Principais hipóteses locais: ${hypotheses.map((item) => item.name).join(', ')}. Base educacional: diretrizes clínicas públicas e revisão por sinais de alarme.`,
    analysisSource: 'local',
  };
}
